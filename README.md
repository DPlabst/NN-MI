...
> Running [example3a_nnmi_v1.0.py](example3a_nnmi_v1.0.py) creates the left subplot below. The receiver performs  matched-filtering, symbol-rate sampling and SDD. We set the noise variance $\sigma^2 = 1$ and vary the average transmit power $P_\text{tx}$. Hence $\mathrm{SNR} := P_\text{tx}$. Achievable information rates are computed for transmit peak powers $\infty$ (red curve), $9\mathrm{dBW}$ and $6\mathrm{dBW}$. 

> Running [example3b_nnmi_v1.0.py](example3b_nnmi_v1.0.py) creates the right subplot below. The receiver performs $4$-fold oversampling and SIC with up to three stages. For higher transmit powers, $f(x)$ broadens the transmit signal spectrum. Hence, oversampling at the receiver increases information rates.
