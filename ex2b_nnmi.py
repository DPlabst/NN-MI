...
szNNvec = np.array([8, 70, 70])

# NN Training parameters
lr = 0.005  # Learning rate
Ni = 30000  # Number of iterations for training
T_rnn_raw = 8*128  # Use approximate T_rnn inputs for training (ceil-ed later)
n_batch =  2*64  # Batches for SGD: Tensor with Dimensions: nBatch x T_rnn x sz[0]
n = 20000  # Set approximate number of validation symbols (ceil-ed later)
n_frames = 500  # Number of frames for final verification
...
n_sched = 400  # Number of training iterations after which LR scheduler is called
...
N_sim = 1  # Oversampling for simulation
...
tx_rolloff = 0.0  # Creates delta pulses (for symbol-spaced simulations)
# Raised-cosine pulse
...
# * --------- RX ADC FILTER h[u] -------------
h = np.array([1])  # delta[k]
rx_cutoff = tx_rolloff  # Relative to N_sim
# RX front-end filter
# h, _ = comsys.gen_ps_rrcos(
#     N_sim=N_sim, N_span=151, alph_roll=tx_rolloff
# )  # Matched filter (scaling does not change SNR)
...
if __name__ == "__main__":
    ...
    filename = "results/ex2b_" + processed[0][2]  # Filename
